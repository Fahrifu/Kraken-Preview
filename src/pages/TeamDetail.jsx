...
export default function TeamDetail(){
  ...
  const tier = String(team.tier || 'division').toUpperCase();
  return <main className="page-main">
    <section className="division-hero"><div className="division-watermark">{team.short || team.name?.slice(0,2).toUpperCase() || 'KR'}</div><div className="division-copy"><Link className="back-link" to="/teams"><ArrowLeft size={16}/> ALL TEAMS</Link><Eyebrow>{tier} DIVISION // {team.region || 'TBD'}</Eyebrow><h1>{team.name || 'Kraken Division'}</h1><p>{team.summary || 'Division information is being updated.'}</p><div className="division-badges"><span>{team.record || '0-0'} RECORD</span><span>{team.ranking || 'UNRANKED'}</span></div></div><img src="/kraken-logo-transparent.png" alt="UiA Kraken"/></section>
